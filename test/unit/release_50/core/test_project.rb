...
class CoordinateMappingsTestSimple < Test::Unit::TestCase
  ...
  def test_assemblies
    ...
    # Chromosome 20 has 2970 scaffolds
    assert_equal(127, chr20.component_seq_regions('contig').length)
...
class SliceProjectFromAssemblyToComponentForwardStrands < Test::Unit::TestCase
  def setup
    @source_slice_single_contig = Slice.fetch_by_region('chromosome', '4', 96652152, 96654117)
    @target_slices_single_contig = @source_slice_single_scaffold.project('contig')

    @source_slice_two_contigs = Slice.fetch_by_region('chromosome','4', 96652152, 98000000)
    @target_slices_two_contigs = @source_slice_two_scaffolds.project('scaffold')

    @source_slice_four_contigs = Slice.fetch_by_region('chromosome', '4', 96652152, 99000000)
    @target_slices_four_contigs = @source_slice_four_scaffolds.project('scaffold')
